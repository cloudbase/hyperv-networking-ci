...
result_init=$?
echo "$ZUUL_PROJECT;$ZUUL_BRANCH;$jen_date;$ZUUL_CHANGE;$ZUUL_PATCHSET;init;$result_init" >> /home/jenkins-slave/networking-hyperv-statistics.log
echo "Init job finished with exit code $result_init"

if [ $result_init -eq 0 ]; then
    jen_date=$(date +%d/%m/%Y-%H:%M)
    if [[ ! -z $RUN_TESTS ]] &&  [[ $RUN_TESTS == "yes" ]]; then
        /usr/local/src/hyperv-networking-ci/jobs/run_tests.sh 2>&1
        result_tempest=$?
...
jen_date=$(date +%d/%m/%Y-%H:%M)
/usr/local/src/hyperv-networking-ci/jobs/run_collect.sh 
result_collect=$?
